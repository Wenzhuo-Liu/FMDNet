...
checkpoint_dir=r'F:\UAH\FAE-MFDriveNet\attention_self\model_save_motor3'
...
def main(use_cuda=True, EPOCHS=200, batch_size=8, model_name="avenet.pt"):
	...
	dataset = Mydata(img_path=r'F:\UAH\UAH_DATA\motor_dataset\train\image',
    txt_path=r'F:\UAH\UAH_DATA\motor_dataset\train\txt',
	label_path=r'F:\UAH\UAH_DATA\motor\train.txt')

	valdataset = Mydata(img_path=r'F:\UAH\UAH_DATA\motor_dataset\val\image',
    txt_path=r'F:\UAH\UAH_DATA\motor_dataset\val\txt',
	label_path=r'F:\UAH\UAH_DATA\motor\val.txt')
		...
		for epoch in range(EPOCHS):
			...
			for subepoch, (img, inc, kfx, kfy, kfz, aud, rol, yaw, out) in enumerate(dataloader):
				...
				if subepoch%400 == 0:
					...
					with open(file=r"F:\UAH\FAE-MFDriveNet\attention_self\log_motor\train3.txt", mode="a+") as f:
			...
			for sepoch,(img, inc, kfx, kfy, kfz, aud, rol, yaw, out) in enumerate(valdataloader):
				...
				if sepoch%400==0:
					...
					with open(file=r"F:\UAH\FAE-MFDriveNet\attention_self\log_motor\val3.txt", mode="a+") as f:
...
def test(use_cuda=True, batch_size=8, model_name=r"F:\UAH\FAE-MFDriveNet\attention_self\model_save_motor3\avenet.pt"):
	model = getAVENet(use_cuda)
	if os.path.exists(model_name):
		model.load_state_dict(torch.load(model_name))
		print("Loading from previous checkpoint.")
	...
	# testdataset = Mydata(img_path='/root/autodl-tmp/motor_dataset/test/image/',
    # txt_path='/root/autodl-tmp/motor_dataset/test/txt/',
	# label_path='/root/autodl-tmp/motor/test.txt')

	testdataset = Mydata(img_path=r'F:\UAH\UAH_DATA\motor_dataset\test\image',
    txt_path=r'F:\UAH\UAH_DATA\motor_dataset\test\txt',
	label_path=r'F:\UAH\UAH_DATA\motor\test.txt')
	...
	for sepoch, (img, inc, kfx, kfy, kfz, aud, rol, yaw, out) in enumerate(testdataloader):
		...
		if sepoch % 300 == 0:
			...
			with open(file=r"F:\UAH\FAE-MFDriveNet\attention_self\log_motor\test3.txt", mode="a+") as f:
