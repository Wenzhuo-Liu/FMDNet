...
# from AVENet_yuan1 import *
from AVENet_eight import *
import shutil
import time
import argparse
...
checkpoint_dir=r'F:\UAH\FAE-MFDriveNet\attention_self\model_save_total3'
...
def main(use_cuda=True, EPOCHS=200, batch_size=8, model_name="avenet.pt"):
	...
	# dataset = Mydata(img_speed_path='/root/autodl-tmp/total/train.txt',
    # img_path='/root/autodl-tmp/dataset_total/train/img/',
    # speed_path='/root/autodl-tmp/dataset_total/train/speed/')

	# valdataset = Mydata(img_speed_path='/root/autodl-tmp/total/val.txt',
    # img_path='/root/autodl-tmp/dataset_total/val/img/',
    # speed_path='/root/autodl-tmp/dataset_total/val/speed/')

	dataset = Mydata(img_path=r'F:\UAH\UAH_DATA\total_dataset\train\image',
    txt_path=r'F:\UAH\UAH_DATA\total_dataset\train\txt',
	label_path=r'F:\UAH\UAH_DATA\total\train.txt')

	valdataset = Mydata(img_path=r'F:\UAH\UAH_DATA\total_dataset\val\image',
    txt_path=r'F:\UAH\UAH_DATA\total_dataset\val\txt',
	label_path=r'F:\UAH\UAH_DATA\total\val.txt')
	...
	try:
		...
		for epoch in range(EPOCHS):
			...
			for subepoch, (img, inc, kfx, kfy, kfz, aud, rol, yaw, out) in enumerate(dataloader):
				# print(subepoch)
				# print(img.shape)
				# print(img.shape)
				# print(aud.shape)
				...
				inc = torch.Tensor(inc.numpy()[idx, :, :, :])
				kfx = torch.Tensor(kfx.numpy()[idx, :, :, :])
				kfy = torch.Tensor(kfy.numpy()[idx, :, :, :])
				kfz = torch.Tensor(kfz.numpy()[idx, :, :, :])
				aud = torch.Tensor(aud.numpy()[idx, :, :, :])
				rol = torch.Tensor(rol.numpy()[idx, :, :, :])
				yaw = torch.Tensor(yaw.numpy()[idx, :, :, :])
				...
				# img=img.to(device)
				# aud=aud.to(device)
				o, _, _, _, _, _, _, _, _= model(img,inc,kfx,kfy,kfz,aud,rol,yaw)
				# if subepoch%400 == 0:
				# 	print(o)
				# 	print(out)
				...
				if subepoch%400 == 0:
					...
					with open(file=r"F:\UAH\FAE-MFDriveNet\attention_self\log_total\train3.txt", mode="a+") as f:
			...
			model.eval()
			for sepoch,(img, inc, kfx, kfy, kfz, aud, rol, yaw, out) in enumerate(valdataloader):
				out = out.squeeze(1)
				idx = (out != 3).numpy().astype(bool)
				if idx.sum() == 0:
					continue
				...
				inc = Variable(inc, volatile=True)
				kfx = Variable(kfx, volatile=True)
				kfy = Variable(kfy, volatile=True)
				kfz = Variable(kfz, volatile=True)
				aud = Variable(aud, volatile=True)
				rol = Variable(rol, volatile=True)
				yaw = Variable(yaw, volatile=True)
				...
				if sepoch%400==0:
					...
					with open(file=r"F:\UAH\FAE-MFDriveNet\attention_self\log_total\val3.txt", mode="a+") as f:
...
def test(use_cuda=True, batch_size=8, model_name="/root/FAE-MFDriveNet/attention_self/model_save_total2/lowest_loss.pt"):
	model = getAVENet(use_cuda)
	if os.path.exists(model_name):
		model.load_state_dict(torch.load(model_name))
		print("Loading from previous checkpoint.")

	# testdataset = Mydata(img_speed_path='/root/autodl-tmp/total/test.txt',
	#  img_path='/root/autodl-tmp/dataset_total/test/img/',
	#  speed_path='/root/autodl-tmp/dataset_total/test/speed/')

	# testdataset = Mydata(img_path='/root/autodl-tmp/total_dataset/test/image/',
    # txt_path='/root/autodl-tmp/total_dataset/test/txt/',
	# label_path='/root/autodl-tmp/total/test.txt')

	testdataset = Mydata(img_path=r'F:\UAH\UAH_DATA\total_dataset\test\image',
    txt_path=r'F:\UAH\UAH_DATA\total_dataset\test\txt',
	label_path=r'F:\UAH\UAH_DATA\total\test.txt')
	...
	model.eval()
	for sepoch, (img, inc, kfx, kfy, kfz, aud, rol, yaw, out) in enumerate(testdataloader):
		out = out.squeeze(1)
		idx = (out != 3).numpy().astype(bool)
		if idx.sum() == 0:
			continue
		...
		if sepoch % 300 == 0:
			...
			with open(file=r"F:\UAH\FAE-MFDriveNet\attention_self\log_total\test3.txt", mode="a+") as f:
