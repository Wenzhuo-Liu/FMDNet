...
checkpoint_dir='/root/FAE-MFDriveNet/attention_self/model_save_second/'
...
def main(use_cuda=True, EPOCHS=200, batch_size=8, model_name="avenet.pt"):
	...
	dataset = Mydata(img_speed_path='/root/autodl-tmp/second/train.txt',
    img_path='/root/autodl-tmp/second_dataset/train/img/',
    speed_path='/root/autodl-tmp/second_dataset/train/speed/')

	valdataset = Mydata(img_speed_path='/root/autodl-tmp/second/val.txt',
    img_path='/root/autodl-tmp/second_dataset/val/img/',
    speed_path='/root/autodl-tmp/second_dataset/val/speed/')
		...
		for epoch in range(EPOCHS):
			...
			for subepoch, (img, aud, out) in enumerate(dataloader):
				...
				if subepoch%400 == 0:
					...
					with open(file="/root/FAE-MFDriveNet/attention_self/log_second/train.txt", mode="a+") as f:
			...
			for sepoch,(img, aud, out) in enumerate(valdataloader):
				...
				if sepoch%400==0:
					...
					with open(file="/root/FAE-MFDriveNet/attention_self/log_second/val.txt", mode="a+") as f:
...
def test(use_cuda=True, batch_size=8, model_name="/root/FAE-MFDriveNet/attention_self/model_save_second/lowest_loss.pt"):
	model = getAVENet(use_cuda)
	if os.path.exists(model_name):
		model.load_state_dict(torch.load(model_name))
		print("Loading from previous checkpoint.")

	testdataset = Mydata(img_speed_path='/root/autodl-tmp/second/test.txt',
	 img_path='/root/autodl-tmp/second_dataset/test/img/',
	 speed_path='/root/autodl-tmp/second_dataset/test/speed/')
	...
	for sepoch, (img, aud, out) in enumerate(testdataloader):
		...
		if sepoch % 300 == 0:
			...
			with open(file="/root/FAE-MFDriveNet/attention_self/log_second/test.txt", mode="a+") as f:
